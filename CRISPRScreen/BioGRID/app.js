...
function startHeroAnimation() {
  if (!els.heroCanvas || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  const canvas = els.heroCanvas;
  const ctx = canvas.getContext("2d");
  const nodes = Array.from({ length: 42 }, (_, index) => ({
    x: (index * 97) % 1000,
    y: (index * 173) % 420,
    vx: 0.18 + ((index % 5) * 0.025),
    vy: index % 2 ? 0.08 : -0.06,
    phase: index * 0.37,
  }));
  ...
  function draw(time) {
    ...
    nodes.forEach((node) => {
      node.x = (node.x + node.vx) % width;
      node.y += node.vy + Math.sin(time * 0.001 + node.phase) * 0.035;
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      for (let j = i + 1; j < nodes.length; j += 1) {
        ...
        if (distance < 145) {
          ctx.globalAlpha = (1 - distance / 145) * 0.28;
          ctx.strokeStyle = i % 3 === 0 ? "#c95c41" : "#226b80";
          ctx.lineWidth = 1;
          ctx.beginPath();
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
    ...
    nodes.forEach((node, index) => {
      ...
      ctx.fillStyle = index % 4 === 0 ? "#bb8a1e" : index % 3 === 0 ? "#6b4c7a" : "#16817a";
      ctx.beginPath();
      ctx.arc(node.x, node.y, 2.2 + (index % 3), 0, Math.PI * 2);
      ctx.fill();
    });
    ctx.globalAlpha = 1;
